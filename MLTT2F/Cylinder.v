...
Module Export CylinderHIT.
  ...
  Axiom FibrantF_Cyl : forall {Δ} {X Y : Δ -> Type}
                        {FibX: FibrantF X} {FibY: FibrantF Y}
                        {f: forall x, X x -> Y x},
      FibrantF2 (fun δ (y : Y δ) => Cyl (f δ) y).
  
  Instance Fibrant_Cyl {X Y} {FibX: Fibrant X} {FibY: Fibrant Y}
           {f: X -> Y} : FibrantF (Cyl f)
    := @FibrantF_compose _ _ (@FibrantF_Cyl unit _ _ _ _ (fun _ => f)) _ (fun y => (tt; y)).
